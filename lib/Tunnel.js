...
module.exports = class Tunnel extends EventEmitter {
  ...
  _init(cb) {
    ...
    const localProtocol = opt.type === 'tcp' ? 'tcp' : (opt.local_https ? 'https' : 'http');
    ...
    // Add type and tcp_port params for TCP tunnels
    if (opt.type && opt.type === 'tcp') {
      const separator = uri.includes('?') ? '&' : '?';
      uri += `${separator}type=tcp`;
      if (opt.tcpPort) {
        uri += `&tcp_port=${opt.tcpPort}`;
      }
    }
