...
class TunnelManager extends EventEmitter {
   ...
   * @param {Array<{port: number, id: string, local_host?: string, type?: 'http'|'tcp'|'udp', tcpPort?: number, udpPort?: number, maxConn?: number}>} opts.tunnels - Tunnel configs
  ...
  _connectSSE() {
    ...
    const req = lib.get(sseUrl, { headers }, (res) => {
      ...
      // Server heartbeats every 15s. If we receive nothing for 45s the connection
      // is considered dead (covers silent NAT/firewall drops where no FIN/RST arrives).
      // Any received byte — including heartbeat comments — resets this timer.
      if (res.socket) {
        res.socket.setKeepAlive(true, 30000);
        res.socket.setTimeout(45000);
        res.socket.once('timeout', () => {
          debug('SSE socket timeout (no data for 45s), assuming connection is dead');
          res.socket.destroy(new Error('SSE idle timeout'));
        });
      }
  ...
  async _openTunnel(id) {
    ...
    try {
      ...
      if (tunnel.type === 'tcp' || tunnel.type === 'udp') {
        debug('Tunnel %s opened (%s, public port: %s)', id, tunnel.type.toUpperCase(), tunnel.publicPort);
      } else {
